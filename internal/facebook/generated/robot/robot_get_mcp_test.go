...
func TestToolRobot_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolRobot_get()
	if tool.Name != "robot_get" {
		t.Errorf("expected tool name robot_get, got %s", tool.Name)
	}
}

func TestToolRobot_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleRobot_get(ctx, request)
