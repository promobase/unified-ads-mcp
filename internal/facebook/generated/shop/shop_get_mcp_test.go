...
func TestToolShop_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolShop_get()
	if tool.Name != "shop_get" {
		t.Errorf("expected tool name shop_get, got %s", tool.Name)
	}
}

func TestToolShop_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleShop_get(ctx, request)
