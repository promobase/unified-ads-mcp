...
// ToolAdimage_get returns the MCP tool definition for adimage_get
func ToolAdimage_get() mcp.Tool {
	...
	return mcp.NewTool("adimage_get",
...
// HandleAdimage_get handles the adimage_get tool with context-based auth
func HandleAdimage_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adimage_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adimage_get: %v", err)), nil
	}
...
// Adimage_get performs GET  for AdImage
func Adimage_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
