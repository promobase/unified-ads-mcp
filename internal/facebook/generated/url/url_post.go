...
// HandleUrl_post handles the url_post tool with context-based auth
func HandleUrl_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Url_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute url_post: %v", err)), nil
	}
...
// Url_post performs POST  for URL
func Url_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
