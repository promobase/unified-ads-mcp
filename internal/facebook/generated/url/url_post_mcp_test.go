...
func TestToolUrl_post_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolUrl_post()
	if tool.Name != "url_post" {
		t.Errorf("expected tool name url_post, got %s", tool.Name)
	}
}

func TestToolUrl_post_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleUrl_post(ctx, request)
