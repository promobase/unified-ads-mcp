...
// ToolUrl_get returns the MCP tool definition for url_get
func ToolUrl_get() mcp.Tool {
	// Available fields for URL: engagement, id, og_object, ownership_permissions, scopes

	return mcp.NewTool("url_get",
...
// HandleUrl_get handles the url_get tool with context-based auth
func HandleUrl_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Url_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute url_get: %v", err)), nil
	}
...
// Url_get performs GET  for URL
func Url_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
