...
func TestToolUrl_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolUrl_get()
	if tool.Name != "url_get" {
		t.Errorf("expected tool name url_get, got %s", tool.Name)
	}
}

func TestToolUrl_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleUrl_get(ctx, request)
