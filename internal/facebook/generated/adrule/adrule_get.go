...
// ToolAdrule_get returns the MCP tool definition for adrule_get
func ToolAdrule_get() mcp.Tool {
...
// HandleAdrule_get handles the adrule_get tool with context-based auth
func HandleAdrule_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adrule_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adrule_get: %v", err)), nil
	}
...
// Adrule_get performs GET  for AdRule
func Adrule_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
