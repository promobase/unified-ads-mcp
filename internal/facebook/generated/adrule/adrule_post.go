...
// ToolAdrule_post returns the MCP tool definition for adrule_post
func ToolAdrule_post() mcp.Tool {

	// Params object accepts: evaluation_spec (Object), execution_spec (Object), name (string), schedule_spec (Object), status (adrule_status)
	return mcp.NewTool("adrule_post",
...
// HandleAdrule_post handles the adrule_post tool with context-based auth
func HandleAdrule_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adrule_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adrule_post: %v", err)), nil
	}
...
// Adrule_post performs POST  for AdRule
func Adrule_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
