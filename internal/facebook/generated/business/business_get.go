...
// ToolBusiness_get returns the MCP tool definition for business_get
func ToolBusiness_get() mcp.Tool {
	...
	return mcp.NewTool("business_get",
...
// HandleBusiness_get handles the business_get tool with context-based auth
func HandleBusiness_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Business_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute business_get: %v", err)), nil
	}
...
// Business_get performs GET  for Business
func Business_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
