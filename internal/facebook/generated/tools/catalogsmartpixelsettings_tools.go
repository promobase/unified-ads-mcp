// Code generated by Facebook Business API codegen. DO NOT EDIT.

package tools

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/mark3labs/mcp-go/mcp"
	"unified-ads-mcp/internal/facebook/generated/client"
	"unified-ads-mcp/internal/shared"
)

// GetCatalogSmartPixelSettingsTools returns MCP tools for CatalogSmartPixelSettings
func GetCatalogSmartPixelSettingsTools(accessToken string) []mcp.Tool {
	var tools []mcp.Tool

	// catalogsmartpixelsettings_get_ tool
	catalogsmartpixelsettings_get_Tool := mcp.NewTool("catalogsmartpixelsettings_get_",
		mcp.WithDescription("GET  for CatalogSmartPixelSettings"),
		mcp.WithString("access_token",
			mcp.Required(),
			mcp.Description("Facebook access token for authentication"),
		),
	)
	tools = append(tools, catalogsmartpixelsettings_get_Tool)

	return tools
}

// GetCatalogSmartPixelSettingsToolsWithoutAuth returns MCP tools for CatalogSmartPixelSettings without access_token parameter
func GetCatalogSmartPixelSettingsToolsWithoutAuth() []mcp.Tool {
	var tools []mcp.Tool

	// catalogsmartpixelsettings_get_ tool
	catalogsmartpixelsettings_get_Tool := mcp.NewTool("catalogsmartpixelsettings_get_",
		mcp.WithDescription("GET  for CatalogSmartPixelSettings"),
	)
	tools = append(tools, catalogsmartpixelsettings_get_Tool)

	return tools
}

// CatalogSmartPixelSettings handlers

// HandleCatalogsmartpixelsettings_get_ handles the catalogsmartpixelsettings_get_ tool
func HandleCatalogsmartpixelsettings_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token
	accessToken, err := request.RequireString("access_token")
	if err != nil {
		return mcp.NewToolResultError("missing required parameter: access_token"), nil
	}

	// Create client
	client := client.NewCatalogSmartPixelSettingsClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Catalogsmartpixelsettings_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute catalogsmartpixelsettings_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}

// Context-aware handlers

// HandleContextCatalogsmartpixelsettings_get_ handles the catalogsmartpixelsettings_get_ tool with context-based auth
func HandleContextCatalogsmartpixelsettings_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token from context
	accessToken, ok := shared.FacebookAccessTokenFromContext(ctx)
	if !ok {
		return mcp.NewToolResultError("Facebook access token not found in context"), nil
	}

	// Create client
	client := client.NewCatalogSmartPixelSettingsClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Catalogsmartpixelsettings_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute catalogsmartpixelsettings_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}
