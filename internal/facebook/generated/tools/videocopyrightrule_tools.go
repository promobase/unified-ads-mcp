// Code generated by Facebook Business API codegen. DO NOT EDIT.

package tools

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/mark3labs/mcp-go/mcp"
	"unified-ads-mcp/internal/facebook/generated/client"
	"unified-ads-mcp/internal/shared"
)

// GetVideoCopyrightRuleTools returns MCP tools for VideoCopyrightRule
func GetVideoCopyrightRuleTools(accessToken string) []mcp.Tool {
	var tools []mcp.Tool

	// videocopyrightrule_get_ tool
	videocopyrightrule_get_Tool := mcp.NewTool("videocopyrightrule_get_",
		mcp.WithDescription("GET  for VideoCopyrightRule"),
		mcp.WithString("access_token",
			mcp.Required(),
			mcp.Description("Facebook access token for authentication"),
		),
	)
	tools = append(tools, videocopyrightrule_get_Tool)

	return tools
}

// GetVideoCopyrightRuleToolsWithoutAuth returns MCP tools for VideoCopyrightRule without access_token parameter
func GetVideoCopyrightRuleToolsWithoutAuth() []mcp.Tool {
	var tools []mcp.Tool

	// videocopyrightrule_get_ tool
	videocopyrightrule_get_Tool := mcp.NewTool("videocopyrightrule_get_",
		mcp.WithDescription("GET  for VideoCopyrightRule"),
	)
	tools = append(tools, videocopyrightrule_get_Tool)

	return tools
}

// VideoCopyrightRule handlers

// HandleVideocopyrightrule_get_ handles the videocopyrightrule_get_ tool
func HandleVideocopyrightrule_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token
	accessToken, err := request.RequireString("access_token")
	if err != nil {
		return mcp.NewToolResultError("missing required parameter: access_token"), nil
	}

	// Create client
	client := client.NewVideoCopyrightRuleClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Videocopyrightrule_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute videocopyrightrule_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}

// Context-aware handlers

// HandleContextVideocopyrightrule_get_ handles the videocopyrightrule_get_ tool with context-based auth
func HandleContextVideocopyrightrule_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token from context
	accessToken, ok := shared.FacebookAccessTokenFromContext(ctx)
	if !ok {
		return mcp.NewToolResultError("Facebook access token not found in context"), nil
	}

	// Create client
	client := client.NewVideoCopyrightRuleClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Videocopyrightrule_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute videocopyrightrule_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}
