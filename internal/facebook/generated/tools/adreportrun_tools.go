...
func GetAdReportRunTools() []mcp.Tool {
	...
	adreportrun_get_Tool := mcp.NewTool("adreportrun_get_",
		...
		mcp.WithString("fields",
			mcp.Description("Comma-separated list of fields to return for AdReportRun objects. Available fields: account_id, async_percent_completion, async_report_url, async_status, date_start, date_stop, emails, error_code, friendly_name, id, is_async_export, is_bookmarked, is_running, schedule_id, time_completed (and 1 more)"),
		),
		mcp.WithNumber("limit",
			mcp.Description("Maximum number of results to return (default: 25, max: 500)"),
		),
