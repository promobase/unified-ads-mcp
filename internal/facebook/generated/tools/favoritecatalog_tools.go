// Code generated by Facebook Business API codegen. DO NOT EDIT.

package tools

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/mark3labs/mcp-go/mcp"
	"unified-ads-mcp/internal/facebook/generated/client"
	"unified-ads-mcp/internal/shared"
)

// GetFavoriteCatalogTools returns MCP tools for FavoriteCatalog
func GetFavoriteCatalogTools(accessToken string) []mcp.Tool {
	var tools []mcp.Tool

	// favoritecatalog_get_ tool
	favoritecatalog_get_Tool := mcp.NewTool("favoritecatalog_get_",
		mcp.WithDescription("GET  for FavoriteCatalog"),
		mcp.WithString("access_token",
			mcp.Required(),
			mcp.Description("Facebook access token for authentication"),
		),
	)
	tools = append(tools, favoritecatalog_get_Tool)

	return tools
}

// GetFavoriteCatalogToolsWithoutAuth returns MCP tools for FavoriteCatalog without access_token parameter
func GetFavoriteCatalogToolsWithoutAuth() []mcp.Tool {
	var tools []mcp.Tool

	// favoritecatalog_get_ tool
	favoritecatalog_get_Tool := mcp.NewTool("favoritecatalog_get_",
		mcp.WithDescription("GET  for FavoriteCatalog"),
	)
	tools = append(tools, favoritecatalog_get_Tool)

	return tools
}

// FavoriteCatalog handlers

// HandleFavoritecatalog_get_ handles the favoritecatalog_get_ tool
func HandleFavoritecatalog_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token
	accessToken, err := request.RequireString("access_token")
	if err != nil {
		return mcp.NewToolResultError("missing required parameter: access_token"), nil
	}

	// Create client
	client := client.NewFavoriteCatalogClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Favoritecatalog_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute favoritecatalog_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}

// Context-aware handlers

// HandleContextFavoritecatalog_get_ handles the favoritecatalog_get_ tool with context-based auth
func HandleContextFavoritecatalog_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token from context
	accessToken, ok := shared.FacebookAccessTokenFromContext(ctx)
	if !ok {
		return mcp.NewToolResultError("Facebook access token not found in context"), nil
	}

	// Create client
	client := client.NewFavoriteCatalogClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Favoritecatalog_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute favoritecatalog_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}
