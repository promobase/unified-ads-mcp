// Code generated by Facebook Business API codegen. DO NOT EDIT.

package tools

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/mark3labs/mcp-go/mcp"
	"unified-ads-mcp/internal/facebook/generated/client"
	"unified-ads-mcp/internal/shared"
)

// GetCanvasTemplateTools returns MCP tools for CanvasTemplate
func GetCanvasTemplateTools(accessToken string) []mcp.Tool {
	var tools []mcp.Tool

	// canvastemplate_get_ tool
	canvastemplate_get_Tool := mcp.NewTool("canvastemplate_get_",
		mcp.WithDescription("GET  for CanvasTemplate"),
		mcp.WithString("access_token",
			mcp.Required(),
			mcp.Description("Facebook access token for authentication"),
		),
	)
	tools = append(tools, canvastemplate_get_Tool)

	return tools
}

// GetCanvasTemplateToolsWithoutAuth returns MCP tools for CanvasTemplate without access_token parameter
func GetCanvasTemplateToolsWithoutAuth() []mcp.Tool {
	var tools []mcp.Tool

	// canvastemplate_get_ tool
	canvastemplate_get_Tool := mcp.NewTool("canvastemplate_get_",
		mcp.WithDescription("GET  for CanvasTemplate"),
	)
	tools = append(tools, canvastemplate_get_Tool)

	return tools
}

// CanvasTemplate handlers

// HandleCanvastemplate_get_ handles the canvastemplate_get_ tool
func HandleCanvastemplate_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token
	accessToken, err := request.RequireString("access_token")
	if err != nil {
		return mcp.NewToolResultError("missing required parameter: access_token"), nil
	}

	// Create client
	client := client.NewCanvasTemplateClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Canvastemplate_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute canvastemplate_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}

// Context-aware handlers

// HandleContextCanvastemplate_get_ handles the canvastemplate_get_ tool with context-based auth
func HandleContextCanvastemplate_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token from context
	accessToken, ok := shared.FacebookAccessTokenFromContext(ctx)
	if !ok {
		return mcp.NewToolResultError("Facebook access token not found in context"), nil
	}

	// Create client
	client := client.NewCanvasTemplateClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Canvastemplate_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute canvastemplate_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}
