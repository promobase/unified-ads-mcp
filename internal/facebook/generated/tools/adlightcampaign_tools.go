// Code generated by Facebook Business API codegen. DO NOT EDIT.

package tools

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/mark3labs/mcp-go/mcp"
	"unified-ads-mcp/internal/facebook/generated/client"
	"unified-ads-mcp/internal/shared"
)

// GetAdLightCampaignTools returns MCP tools for AdLightCampaign
func GetAdLightCampaignTools(accessToken string) []mcp.Tool {
	var tools []mcp.Tool

	// adlightcampaign_get_ tool
	adlightcampaign_get_Tool := mcp.NewTool("adlightcampaign_get_",
		mcp.WithDescription("GET  for AdLightCampaign"),
		mcp.WithString("access_token",
			mcp.Required(),
			mcp.Description("Facebook access token for authentication"),
		),
	)
	tools = append(tools, adlightcampaign_get_Tool)

	return tools
}

// GetAdLightCampaignToolsWithoutAuth returns MCP tools for AdLightCampaign without access_token parameter
func GetAdLightCampaignToolsWithoutAuth() []mcp.Tool {
	var tools []mcp.Tool

	// adlightcampaign_get_ tool
	adlightcampaign_get_Tool := mcp.NewTool("adlightcampaign_get_",
		mcp.WithDescription("GET  for AdLightCampaign"),
	)
	tools = append(tools, adlightcampaign_get_Tool)

	return tools
}

// AdLightCampaign handlers

// HandleAdlightcampaign_get_ handles the adlightcampaign_get_ tool
func HandleAdlightcampaign_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token
	accessToken, err := request.RequireString("access_token")
	if err != nil {
		return mcp.NewToolResultError("missing required parameter: access_token"), nil
	}

	// Create client
	client := client.NewAdLightCampaignClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Adlightcampaign_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adlightcampaign_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}

// Context-aware handlers

// HandleContextAdlightcampaign_get_ handles the adlightcampaign_get_ tool with context-based auth
func HandleContextAdlightcampaign_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token from context
	accessToken, ok := shared.FacebookAccessTokenFromContext(ctx)
	if !ok {
		return mcp.NewToolResultError("Facebook access token not found in context"), nil
	}

	// Create client
	client := client.NewAdLightCampaignClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Adlightcampaign_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adlightcampaign_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}
