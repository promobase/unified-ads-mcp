// Code generated by Facebook Business API codegen. DO NOT EDIT.

package tools

import (
	"context"
	"encoding/json"
	"fmt"

	"github.com/mark3labs/mcp-go/mcp"
	"unified-ads-mcp/internal/facebook/generated/client"
	"unified-ads-mcp/internal/shared"
)

// GetArAdsDataContainerTools returns MCP tools for ArAdsDataContainer
func GetArAdsDataContainerTools(accessToken string) []mcp.Tool {
	var tools []mcp.Tool

	// aradsdatacontainer_get_ tool
	aradsdatacontainer_get_Tool := mcp.NewTool("aradsdatacontainer_get_",
		mcp.WithDescription("GET  for ArAdsDataContainer"),
		mcp.WithString("access_token",
			mcp.Required(),
			mcp.Description("Facebook access token for authentication"),
		),
	)
	tools = append(tools, aradsdatacontainer_get_Tool)

	return tools
}

// GetArAdsDataContainerToolsWithoutAuth returns MCP tools for ArAdsDataContainer without access_token parameter
func GetArAdsDataContainerToolsWithoutAuth() []mcp.Tool {
	var tools []mcp.Tool

	// aradsdatacontainer_get_ tool
	aradsdatacontainer_get_Tool := mcp.NewTool("aradsdatacontainer_get_",
		mcp.WithDescription("GET  for ArAdsDataContainer"),
	)
	tools = append(tools, aradsdatacontainer_get_Tool)

	return tools
}

// ArAdsDataContainer handlers

// HandleAradsdatacontainer_get_ handles the aradsdatacontainer_get_ tool
func HandleAradsdatacontainer_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token
	accessToken, err := request.RequireString("access_token")
	if err != nil {
		return mcp.NewToolResultError("missing required parameter: access_token"), nil
	}

	// Create client
	client := client.NewArAdsDataContainerClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Aradsdatacontainer_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute aradsdatacontainer_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}

// Context-aware handlers

// HandleContextAradsdatacontainer_get_ handles the aradsdatacontainer_get_ tool with context-based auth
func HandleContextAradsdatacontainer_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Get access token from context
	accessToken, ok := shared.FacebookAccessTokenFromContext(ctx)
	if !ok {
		return mcp.NewToolResultError("Facebook access token not found in context"), nil
	}

	// Create client
	client := client.NewArAdsDataContainerClient(accessToken)

	// Build arguments map
	args := make(map[string]interface{})

	// Call the client method
	result, err := client.Aradsdatacontainer_get_(args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute aradsdatacontainer_get_: %v", err)), nil
	}

	// Return the result as JSON
	resultJSON, err := json.Marshal(result)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to marshal result: %v", err)), nil
	}

	return mcp.NewToolResultText(string(resultJSON)), nil
}
