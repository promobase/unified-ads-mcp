...
// ToolVideocopyright_post returns the MCP tool definition for videocopyright_post
func ToolVideocopyright_post() mcp.Tool {
	...
	return mcp.NewTool("videocopyright_post",
...
// HandleVideocopyright_post handles the videocopyright_post tool with context-based auth
func HandleVideocopyright_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Videocopyright_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute videocopyright_post: %v", err)), nil
	}
...
// Videocopyright_post performs POST  for VideoCopyright
func Videocopyright_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
