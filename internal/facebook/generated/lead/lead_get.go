...
// ToolLead_get returns the MCP tool definition for lead_get
func ToolLead_get() mcp.Tool {
...
// HandleLead_get handles the lead_get tool with context-based auth
func HandleLead_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Lead_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute lead_get: %v", err)), nil
	}
...
// Lead_get performs GET  for Lead
func Lead_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
