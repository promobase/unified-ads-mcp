...
func TestToolLead_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolLead_get()
	if tool.Name != "lead_get" {
		t.Errorf("expected tool name lead_get, got %s", tool.Name)
	}
}

func TestToolLead_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleLead_get(ctx, request)
