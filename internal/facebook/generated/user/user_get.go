...
// ToolUser_get returns the MCP tool definition for user_get
func ToolUser_get() mcp.Tool {
...
// HandleUser_get handles the user_get tool with context-based auth
func HandleUser_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := User_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute user_get: %v", err)), nil
	}
...
// User_get performs GET  for User
func User_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
