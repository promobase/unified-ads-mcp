...
// RegisterUserListFriendsHandler registers the user_list_friends tool
func RegisterUserListFriendsHandler(s *server.MCPServer) error {
	...
	s.AddTool(tool, UserListFriendsHandler)
	return nil
}

// UserListFriendsHandler handles the user_list_friends tool
func UserListFriendsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args user_list_friendsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/friends", args.ID)
