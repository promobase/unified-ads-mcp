...
// RegisterUserListAvatarsHandler registers the user_list_avatars tool
func RegisterUserListAvatarsHandler(s *server.MCPServer) error {
	...
	s.AddTool(tool, UserListAvatarsHandler)
	return nil
}

// UserListAvatarsHandler handles the user_list_avatars tool
func UserListAvatarsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args user_list_avatarsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/avatars", args.ID)
