...
// RegisterUserListAssignedApplicationsHandler registers the user_list_assigned_applications tool
func RegisterUserListAssignedApplicationsHandler(s *server.MCPServer) error {
	...
	s.AddTool(tool, UserListAssignedApplicationsHandler)
	return nil
}

// UserListAssignedApplicationsHandler handles the user_list_assigned_applications tool
func UserListAssignedApplicationsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args user_list_assigned_applicationsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/assigned_applications", args.ID)
