...
// RegisterUserListPersonalAdAccountsHandler registers the user_list_personal_ad_accounts tool
func RegisterUserListPersonalAdAccountsHandler(s *server.MCPServer) error {
	...
	s.AddTool(tool, UserListPersonalAdAccountsHandler)
	return nil
}

// UserListPersonalAdAccountsHandler handles the user_list_personal_ad_accounts tool
func UserListPersonalAdAccountsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args user_list_personal_ad_accountsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/personal_ad_accounts", args.ID)
