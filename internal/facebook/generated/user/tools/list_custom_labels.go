...
// RegisterUserListCustomLabelsHandler registers the user_list_custom_labels tool
func RegisterUserListCustomLabelsHandler(s *server.MCPServer) error {
	...
	s.AddTool(tool, UserListCustomLabelsHandler)
	return nil
}

// UserListCustomLabelsHandler handles the user_list_custom_labels tool
func UserListCustomLabelsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args user_list_custom_labelsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/custom_labels", args.ID)
