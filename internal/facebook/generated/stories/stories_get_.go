...
// ToolStories_get_ returns the MCP tool definition for stories_get_
func ToolStories_get_() mcp.Tool {
	// Available fields for Stories: creation_time, media_id, media_type, post_id, status, url

	return mcp.NewTool("stories_get_",
...
func HandleStories_get_(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Stories_get_(accessToken, args)
	if err != nil {
...
// Stories_get_ performs GET  for Stories
func Stories_get_(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
