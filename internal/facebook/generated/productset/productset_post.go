...
// ToolProductset_post returns the MCP tool definition for productset_post
func ToolProductset_post() mcp.Tool {
	...
	return mcp.NewTool("productset_post",
...
// HandleProductset_post handles the productset_post tool with context-based auth
func HandleProductset_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Productset_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute productset_post: %v", err)), nil
	}
...
// Productset_post performs POST  for ProductSet
func Productset_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
