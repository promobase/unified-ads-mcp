...
// ToolGroup_post returns the MCP tool definition for group_post
func ToolGroup_post() mcp.Tool {
...
// HandleGroup_post handles the group_post tool with context-based auth
func HandleGroup_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Group_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute group_post: %v", err)), nil
	}
...
// Group_post performs POST  for Group
func Group_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
