...
func TestToolGroup_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolGroup_get()
	if tool.Name != "group_get" {
		t.Errorf("expected tool name group_get, got %s", tool.Name)
	}
}

func TestToolGroup_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleGroup_get(ctx, request)
