...
func TestToolGroup_post_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolGroup_post()
	if tool.Name != "group_post" {
		t.Errorf("expected tool name group_post, got %s", tool.Name)
	}
}

func TestToolGroup_post_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleGroup_post(ctx, request)
