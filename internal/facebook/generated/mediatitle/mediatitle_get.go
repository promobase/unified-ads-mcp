...
// ToolMediatitle_get returns the MCP tool definition for mediatitle_get
func ToolMediatitle_get() mcp.Tool {
	...
	return mcp.NewTool("mediatitle_get",
...
// HandleMediatitle_get handles the mediatitle_get tool with context-based auth
func HandleMediatitle_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Mediatitle_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute mediatitle_get: %v", err)), nil
	}
...
// Mediatitle_get performs GET  for MediaTitle
func Mediatitle_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
