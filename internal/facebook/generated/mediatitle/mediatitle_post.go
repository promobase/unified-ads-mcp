...
// ToolMediatitle_post returns the MCP tool definition for mediatitle_post
func ToolMediatitle_post() mcp.Tool {
	...
	return mcp.NewTool("mediatitle_post",
...
// HandleMediatitle_post handles the mediatitle_post tool with context-based auth
func HandleMediatitle_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Mediatitle_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute mediatitle_post: %v", err)), nil
	}
...
// Mediatitle_post performs POST  for MediaTitle
func Mediatitle_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
