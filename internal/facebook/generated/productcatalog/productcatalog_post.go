...
// ToolProductcatalog_post returns the MCP tool definition for productcatalog_post
func ToolProductcatalog_post() mcp.Tool {
	...
	return mcp.NewTool("productcatalog_post",
...
// HandleProductcatalog_post handles the productcatalog_post tool with context-based auth
func HandleProductcatalog_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Productcatalog_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute productcatalog_post: %v", err)), nil
	}
...
// Productcatalog_post performs POST  for ProductCatalog
func Productcatalog_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
