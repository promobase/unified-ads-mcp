...
// ToolPost_get returns the MCP tool definition for post_get
func ToolPost_get() mcp.Tool {
...
// HandlePost_get handles the post_get tool with context-based auth
func HandlePost_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Post_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute post_get: %v", err)), nil
	}
...
// Post_get performs GET  for Post
