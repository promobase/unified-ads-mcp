...
// ToolPost_post returns the MCP tool definition for post_post
func ToolPost_post() mcp.Tool {
...
// HandlePost_post handles the post_post tool with context-based auth
func HandlePost_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Post_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute post_post: %v", err)), nil
	}
...
// Post_post performs POST  for Post
func Post_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
