...
func TestToolPost_post_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolPost_post()
	if tool.Name != "post_post" {
		t.Errorf("expected tool name post_post, got %s", tool.Name)
	}
}

func TestToolPost_post_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandlePost_post(ctx, request)
