...
// ToolDestination_get returns the MCP tool definition for destination_get
func ToolDestination_get() mcp.Tool {
	...
	return mcp.NewTool("destination_get",
...
// HandleDestination_get handles the destination_get tool with context-based auth
func HandleDestination_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Destination_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute destination_get: %v", err)), nil
	}
...
// Destination_get performs GET  for Destination
func Destination_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
