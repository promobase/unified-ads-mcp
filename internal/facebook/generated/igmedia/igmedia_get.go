...
// ToolIgmedia_get returns the MCP tool definition for igmedia_get
func ToolIgmedia_get() mcp.Tool {
	...
	return mcp.NewTool("igmedia_get",
...
// HandleIgmedia_get handles the igmedia_get tool with context-based auth
func HandleIgmedia_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Igmedia_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute igmedia_get: %v", err)), nil
	}
...
// Igmedia_get performs GET  for IGMedia
func Igmedia_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
