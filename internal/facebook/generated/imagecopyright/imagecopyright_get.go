...
// ToolImagecopyright_get returns the MCP tool definition for imagecopyright_get
func ToolImagecopyright_get() mcp.Tool {
	...
	return mcp.NewTool("imagecopyright_get",
...
// HandleImagecopyright_get handles the imagecopyright_get tool with context-based auth
func HandleImagecopyright_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Imagecopyright_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute imagecopyright_get: %v", err)), nil
	}
...
// Imagecopyright_get performs GET  for ImageCopyright
func Imagecopyright_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
