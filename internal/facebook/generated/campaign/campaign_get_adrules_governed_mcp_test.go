...
func TestToolCampaign_get_adrules_governed_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_get_adrules_governed()
	if tool.Name != "campaign_get_adrules_governed" {
		t.Errorf("expected tool name campaign_get_adrules_governed, got %s", tool.Name)
	}
}

func TestToolCampaign_get_adrules_governed_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "campaign_get_adrules_governed",
			...
			// Call handler
			result, err := HandleCampaign_get_adrules_governed(ctx, request)
