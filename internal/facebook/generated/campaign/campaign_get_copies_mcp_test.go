...
func TestToolCampaign_get_copies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_get_copies()
	if tool.Name != "campaign_get_copies" {
		t.Errorf("expected tool name campaign_get_copies, got %s", tool.Name)
	}
}

func TestToolCampaign_get_copies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCampaign_get_copies(ctx, request)
