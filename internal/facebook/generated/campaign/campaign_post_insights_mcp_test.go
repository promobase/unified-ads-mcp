...
func TestToolCampaign_post_insights_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_post_insights()
	if tool.Name != "campaign_post_insights" {
		t.Errorf("expected tool name campaign_post_insights, got %s", tool.Name)
	}
}

func TestToolCampaign_post_insights_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCampaign_post_insights(ctx, request)
