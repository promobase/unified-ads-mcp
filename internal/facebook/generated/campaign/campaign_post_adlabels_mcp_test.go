...
func TestToolCampaign_post_adlabels_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_post_adlabels()
	if tool.Name != "campaign_post_adlabels" {
		t.Errorf("expected tool name campaign_post_adlabels, got %s", tool.Name)
	}
}

func TestToolCampaign_post_adlabels_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCampaign_post_adlabels(ctx, request)
