...
func TestToolCampaign_get_ad_studies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_get_ad_studies()
	if tool.Name != "campaign_get_ad_studies" {
		t.Errorf("expected tool name campaign_get_ad_studies, got %s", tool.Name)
	}
}

func TestToolCampaign_get_ad_studies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCampaign_get_ad_studies(ctx, request)
