...
func TestToolCampaign_get_insights_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_get_insights()
	if tool.Name != "campaign_get_insights" {
		t.Errorf("expected tool name campaign_get_insights, got %s", tool.Name)
	}
}

func TestToolCampaign_get_insights_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCampaign_get_insights(ctx, request)
