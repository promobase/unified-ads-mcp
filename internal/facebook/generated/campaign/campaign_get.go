...
// ToolCampaign_get returns the MCP tool definition for campaign_get
func ToolCampaign_get() mcp.Tool {
	...
	return mcp.NewTool("campaign_get",
...
// HandleCampaign_get handles the campaign_get tool with context-based auth
func HandleCampaign_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Campaign_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute campaign_get: %v", err)), nil
	}
...
// Campaign_get performs GET  for Campaign
func Campaign_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string

	// Extract campaign_id for URL construction
	campaignId, ok := args["campaign_id"].(string)
	if !ok {
		return nil, fmt.Errorf("campaign_id is required for campaign_get")
	}
