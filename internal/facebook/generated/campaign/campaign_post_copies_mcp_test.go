...
func TestToolCampaign_post_copies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCampaign_post_copies()
	if tool.Name != "campaign_post_copies" {
		t.Errorf("expected tool name campaign_post_copies, got %s", tool.Name)
	}
}

func TestToolCampaign_post_copies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCampaign_post_copies(ctx, request)
