...
func TestToolLink_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolLink_get()
	if tool.Name != "link_get" {
		t.Errorf("expected tool name link_get, got %s", tool.Name)
	}
}

func TestToolLink_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleLink_get(ctx, request)
