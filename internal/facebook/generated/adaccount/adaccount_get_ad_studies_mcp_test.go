...
func TestToolAdaccount_get_ad_studies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_ad_studies()
	if tool.Name != "adaccount_get_ad_studies" {
		t.Errorf("expected tool name adaccount_get_ad_studies, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_ad_studies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_ad_studies(ctx, request)
