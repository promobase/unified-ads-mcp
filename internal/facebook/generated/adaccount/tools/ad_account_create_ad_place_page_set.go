...
// ad_account_create_ad_place_page_setArgs defines the typed arguments for ad_account_create_ad_place_page_set
type ad_account_create_ad_place_page_setArgs struct {
...
// RegisterAdAccountCreateAdPlacePageSetHandler registers the ad_account_create_ad_place_page_set tool
func RegisterAdAccountCreateAdPlacePageSetHandler(s *server.MCPServer) error {
	tool := mcp.NewToolWithRawSchema(
		"ad_account_create_ad_place_page_set",
		"Associate ad_place_page_sets with this AdAccount Returns AdPlacePageSet. Required: name, parent_page",
		json.RawMessage(`{"additionalProperties":false,"properties":{"id":{"description":"AdAccount ID","pattern":"^[0-9]+$","type":"string"},"location_types":{"description":"Location Types","items":{"type":"string"},"type":"array"},"name":{"description":"Name","type":"string"},"parent_page":{"description":"Parent Page","type":"string"},"targeted_area_type":{"description":"Targeted Area Type (enum: adaccountad_place_page_sets_targeted_area_type_enum_param)","enum":["CUSTOM_RADIUS","MARKETING_AREA","NONE"],"type":"string"}},"required":["id","name","parent_page"],"type":"object"}`),
	)

	s.AddTool(tool, AdAccountCreateAdPlacePageSetHandler)
	return nil
}

// AdAccountCreateAdPlacePageSetHandler handles the ad_account_create_ad_place_page_set tool
func AdAccountCreateAdPlacePageSetHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args ad_account_create_ad_place_page_setArgs
