...
// ad_account_list_connected_instagram_accountsArgs defines the typed arguments for ad_account_list_connected_instagram_accounts
type ad_account_list_connected_instagram_accountsArgs struct {
...
// RegisterAdAccountListConnectedInstagramAccountsHandler registers the ad_account_list_connected_instagram_accounts tool
func RegisterAdAccountListConnectedInstagramAccountsHandler(s *server.MCPServer) error {
	tool := mcp.NewToolWithRawSchema(
		"ad_account_list_connected_instagram_accounts",
	...
	s.AddTool(tool, AdAccountListConnectedInstagramAccountsHandler)
	return nil
}

// AdAccountListConnectedInstagramAccountsHandler handles the ad_account_list_connected_instagram_accounts tool
func AdAccountListConnectedInstagramAccountsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args ad_account_list_connected_instagram_accountsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/connected_instagram_accounts", args.ID)
