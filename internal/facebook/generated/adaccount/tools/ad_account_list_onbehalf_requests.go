...
// ad_account_list_onbehalf_requestsArgs defines the typed arguments for ad_account_list_onbehalf_requests
type ad_account_list_onbehalf_requestsArgs struct {
...
// RegisterAdAccountListOnbehalfRequestsHandler registers the ad_account_list_onbehalf_requests tool
func RegisterAdAccountListOnbehalfRequestsHandler(s *server.MCPServer) error {
	tool := mcp.NewToolWithRawSchema(
		"ad_account_list_onbehalf_requests",
		"List onbehalf_requests for this AdAccount Returns BusinessOwnedObjectOnBehalfOfRequest.",
		json.RawMessage(`{"additionalProperties":false,"properties":{"after":{"description":"Cursor for pagination (next page)","type":"string"},"before":{"description":"Cursor for pagination (previous page)","type":"string"},"fields":{"description":"Fields to return","items":{"type":"string"},"type":"array"},"id":{"description":"AdAccount ID","pattern":"^[0-9]+$","type":"string"},"limit":{"description":"Maximum number of results","maximum":100,"minimum":1,"type":"integer"},"status":{"description":"Status (enum: adaccountonbehalf_requests_status_enum_param)","enum":["APPROVE","CANCELED","DECLINE","EXPIRED","IN_PROGRESS","PENDING","PENDING_EMAIL_VERIFICATION","PENDING_INTEGRITY_REVIEW"],"type":"string"}},"required":["id"],"type":"object"}`),
	)

	s.AddTool(tool, AdAccountListOnbehalfRequestsHandler)
	return nil
}

// AdAccountListOnbehalfRequestsHandler handles the ad_account_list_onbehalf_requests tool
func AdAccountListOnbehalfRequestsHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var args ad_account_list_onbehalf_requestsArgs
	if err := request.BindArguments(&args); err != nil {
		return common.HandleBindError(err)
	}
	endpoint := fmt.Sprintf("/%s/onbehalf_requests", args.ID)
