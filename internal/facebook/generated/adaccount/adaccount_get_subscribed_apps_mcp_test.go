...
func TestToolAdaccount_get_subscribed_apps_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_subscribed_apps()
	if tool.Name != "adaccount_get_subscribed_apps" {
		t.Errorf("expected tool name adaccount_get_subscribed_apps, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_subscribed_apps_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_subscribed_apps",
			...
			// Call handler
			result, err := HandleAdaccount_get_subscribed_apps(ctx, request)
