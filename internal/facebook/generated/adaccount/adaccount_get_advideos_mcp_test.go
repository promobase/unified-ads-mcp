...
func TestToolAdaccount_get_advideos_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_advideos()
	if tool.Name != "adaccount_get_advideos" {
		t.Errorf("expected tool name adaccount_get_advideos, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_advideos_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_advideos(ctx, request)
