...
func TestToolAdaccount_get_adspixels_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_adspixels()
	if tool.Name != "adaccount_get_adspixels" {
		t.Errorf("expected tool name adaccount_get_adspixels, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_adspixels_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_adspixels(ctx, request)
