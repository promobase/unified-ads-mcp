...
func TestToolAdaccount_get_saved_audiences_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_saved_audiences()
	if tool.Name != "adaccount_get_saved_audiences" {
		t.Errorf("expected tool name adaccount_get_saved_audiences, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_saved_audiences_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_saved_audiences",
			...
			// Call handler
			result, err := HandleAdaccount_get_saved_audiences(ctx, request)
