...
func TestToolAdaccount_post_campaigns_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_campaigns()
	if tool.Name != "adaccount_post_campaigns" {
		t.Errorf("expected tool name adaccount_post_campaigns, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_campaigns_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_campaigns(ctx, request)
