...
func TestToolAdaccount_get_tracking_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_tracking()
	if tool.Name != "adaccount_get_tracking" {
		t.Errorf("expected tool name adaccount_get_tracking, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_tracking_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_tracking(ctx, request)
