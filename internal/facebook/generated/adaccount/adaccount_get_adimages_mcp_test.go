...
func TestToolAdaccount_get_adimages_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_adimages()
	if tool.Name != "adaccount_get_adimages" {
		t.Errorf("expected tool name adaccount_get_adimages, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_adimages_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_adimages(ctx, request)
