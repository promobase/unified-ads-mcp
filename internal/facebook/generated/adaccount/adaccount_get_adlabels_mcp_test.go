...
func TestToolAdaccount_get_adlabels_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_adlabels()
	if tool.Name != "adaccount_get_adlabels" {
		t.Errorf("expected tool name adaccount_get_adlabels, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_adlabels_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_adlabels(ctx, request)
