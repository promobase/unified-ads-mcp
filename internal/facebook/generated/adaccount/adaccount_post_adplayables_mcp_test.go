...
func TestToolAdaccount_post_adplayables_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_adplayables()
	if tool.Name != "adaccount_post_adplayables" {
		t.Errorf("expected tool name adaccount_post_adplayables, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_adplayables_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_post_adplayables",
			...
			// Call handler
			result, err := HandleAdaccount_post_adplayables(ctx, request)
