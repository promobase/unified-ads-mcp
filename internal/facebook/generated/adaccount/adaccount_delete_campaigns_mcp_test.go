...
func TestToolAdaccount_delete_campaigns_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_delete_campaigns()
	if tool.Name != "adaccount_delete_campaigns" {
		t.Errorf("expected tool name adaccount_delete_campaigns, got %s", tool.Name)
	}
}

func TestToolAdaccount_delete_campaigns_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_delete_campaigns",
			...
			// Call handler
			result, err := HandleAdaccount_delete_campaigns(ctx, request)
