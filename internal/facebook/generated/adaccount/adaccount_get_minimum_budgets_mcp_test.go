...
func TestToolAdaccount_get_minimum_budgets_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_minimum_budgets()
	if tool.Name != "adaccount_get_minimum_budgets" {
		t.Errorf("expected tool name adaccount_get_minimum_budgets, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_minimum_budgets_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_minimum_budgets",
			...
			// Call handler
			result, err := HandleAdaccount_get_minimum_budgets(ctx, request)
