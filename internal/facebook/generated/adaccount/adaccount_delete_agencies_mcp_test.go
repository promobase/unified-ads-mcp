...
func TestToolAdaccount_delete_agencies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_delete_agencies()
	if tool.Name != "adaccount_delete_agencies" {
		t.Errorf("expected tool name adaccount_delete_agencies, got %s", tool.Name)
	}
}

func TestToolAdaccount_delete_agencies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_delete_agencies",
			...
			// Call handler
			result, err := HandleAdaccount_delete_agencies(ctx, request)
