...
func TestToolAdaccount_post_assigned_users_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_assigned_users()
	if tool.Name != "adaccount_post_assigned_users" {
		t.Errorf("expected tool name adaccount_post_assigned_users, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_assigned_users_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_post_assigned_users",
			...
			// Call handler
			result, err := HandleAdaccount_post_assigned_users(ctx, request)
