...
func TestToolAdaccount_post_adimages_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_adimages()
	if tool.Name != "adaccount_post_adimages" {
		t.Errorf("expected tool name adaccount_post_adimages, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_adimages_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_adimages(ctx, request)
