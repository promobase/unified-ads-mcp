...
func TestToolAdaccount_get_recommendations_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_recommendations()
	if tool.Name != "adaccount_get_recommendations" {
		t.Errorf("expected tool name adaccount_get_recommendations, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_recommendations_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_recommendations",
			...
			// Call handler
			result, err := HandleAdaccount_get_recommendations(ctx, request)
