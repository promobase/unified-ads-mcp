...
func TestToolAdaccount_post_value_rule_set_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_value_rule_set()
	if tool.Name != "adaccount_post_value_rule_set" {
		t.Errorf("expected tool name adaccount_post_value_rule_set, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_value_rule_set_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_post_value_rule_set",
			...
			// Call handler
			result, err := HandleAdaccount_post_value_rule_set(ctx, request)
