...
func TestToolAdaccount_post_insights_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_insights()
	if tool.Name != "adaccount_post_insights" {
		t.Errorf("expected tool name adaccount_post_insights, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_insights_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_insights(ctx, request)
