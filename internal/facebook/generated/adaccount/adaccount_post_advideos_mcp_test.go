...
func TestToolAdaccount_post_advideos_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_advideos()
	if tool.Name != "adaccount_post_advideos" {
		t.Errorf("expected tool name adaccount_post_advideos, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_advideos_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_advideos(ctx, request)
