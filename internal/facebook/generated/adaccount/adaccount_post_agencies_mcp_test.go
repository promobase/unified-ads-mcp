...
func TestToolAdaccount_post_agencies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_agencies()
	if tool.Name != "adaccount_post_agencies" {
		t.Errorf("expected tool name adaccount_post_agencies, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_agencies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_agencies(ctx, request)
