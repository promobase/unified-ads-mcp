...
func TestToolAdaccount_get_campaigns_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_campaigns()
	if tool.Name != "adaccount_get_campaigns" {
		t.Errorf("expected tool name adaccount_get_campaigns, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_campaigns_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_campaigns(ctx, request)
