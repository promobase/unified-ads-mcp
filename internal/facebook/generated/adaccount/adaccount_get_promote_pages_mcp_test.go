...
func TestToolAdaccount_get_promote_pages_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_promote_pages()
	if tool.Name != "adaccount_get_promote_pages" {
		t.Errorf("expected tool name adaccount_get_promote_pages, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_promote_pages_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_promote_pages",
			...
			// Call handler
			result, err := HandleAdaccount_get_promote_pages(ctx, request)
