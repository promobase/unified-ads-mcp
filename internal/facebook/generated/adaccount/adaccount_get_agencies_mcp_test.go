...
func TestToolAdaccount_get_agencies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_agencies()
	if tool.Name != "adaccount_get_agencies" {
		t.Errorf("expected tool name adaccount_get_agencies, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_agencies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_agencies(ctx, request)
