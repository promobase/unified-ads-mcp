...
func TestToolAdaccount_post_tracking_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_tracking()
	if tool.Name != "adaccount_post_tracking" {
		t.Errorf("expected tool name adaccount_post_tracking, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_tracking_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_tracking(ctx, request)
