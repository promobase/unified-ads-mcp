...
func TestToolAdaccount_get_mcmeconversions_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_mcmeconversions()
	if tool.Name != "adaccount_get_mcmeconversions" {
		t.Errorf("expected tool name adaccount_get_mcmeconversions, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_mcmeconversions_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_mcmeconversions",
			...
			// Call handler
			result, err := HandleAdaccount_get_mcmeconversions(ctx, request)
