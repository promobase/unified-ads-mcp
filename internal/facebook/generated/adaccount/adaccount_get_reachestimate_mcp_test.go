...
func TestToolAdaccount_get_reachestimate_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_reachestimate()
	if tool.Name != "adaccount_get_reachestimate" {
		t.Errorf("expected tool name adaccount_get_reachestimate, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_reachestimate_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_reachestimate",
			...
			// Call handler
			result, err := HandleAdaccount_get_reachestimate(ctx, request)
