...
func TestToolAdaccount_get_adrules_library_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_adrules_library()
	if tool.Name != "adaccount_get_adrules_library" {
		t.Errorf("expected tool name adaccount_get_adrules_library, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_adrules_library_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adaccount_get_adrules_library",
			...
			// Call handler
			result, err := HandleAdaccount_get_adrules_library(ctx, request)
