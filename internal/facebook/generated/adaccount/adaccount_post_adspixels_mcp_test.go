...
func TestToolAdaccount_post_adspixels_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_adspixels()
	if tool.Name != "adaccount_post_adspixels" {
		t.Errorf("expected tool name adaccount_post_adspixels, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_adspixels_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_adspixels(ctx, request)
