...
func TestToolAdaccount_get_insights_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_insights()
	if tool.Name != "adaccount_get_insights" {
		t.Errorf("expected tool name adaccount_get_insights, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_insights_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_insights(ctx, request)
