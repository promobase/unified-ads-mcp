...
func TestToolAdaccount_post_adsets_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_post_adsets()
	if tool.Name != "adaccount_post_adsets" {
		t.Errorf("expected tool name adaccount_post_adsets, got %s", tool.Name)
	}
}

func TestToolAdaccount_post_adsets_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_post_adsets(ctx, request)
