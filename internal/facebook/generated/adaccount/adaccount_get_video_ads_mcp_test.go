...
func TestToolAdaccount_get_video_ads_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_video_ads()
	if tool.Name != "adaccount_get_video_ads" {
		t.Errorf("expected tool name adaccount_get_video_ads, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_video_ads_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_video_ads(ctx, request)
