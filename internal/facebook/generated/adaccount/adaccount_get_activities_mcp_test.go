...
func TestToolAdaccount_get_activities_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_activities()
	if tool.Name != "adaccount_get_activities" {
		t.Errorf("expected tool name adaccount_get_activities, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_activities_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_activities(ctx, request)
