...
func TestToolAdaccount_get_ads_volume_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdaccount_get_ads_volume()
	if tool.Name != "adaccount_get_ads_volume" {
		t.Errorf("expected tool name adaccount_get_ads_volume, got %s", tool.Name)
	}
}

func TestToolAdaccount_get_ads_volume_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdaccount_get_ads_volume(ctx, request)
