...
func TestToolAdset_get_asyncadrequests_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdset_get_asyncadrequests()
	if tool.Name != "adset_get_asyncadrequests" {
		t.Errorf("expected tool name adset_get_asyncadrequests, got %s", tool.Name)
	}
}

func TestToolAdset_get_asyncadrequests_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "adset_get_asyncadrequests",
			...
			// Call handler
			result, err := HandleAdset_get_asyncadrequests(ctx, request)
