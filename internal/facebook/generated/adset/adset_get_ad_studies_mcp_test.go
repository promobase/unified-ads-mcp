...
func TestToolAdset_get_ad_studies_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdset_get_ad_studies()
	if tool.Name != "adset_get_ad_studies" {
		t.Errorf("expected tool name adset_get_ad_studies, got %s", tool.Name)
	}
}

func TestToolAdset_get_ad_studies_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdset_get_ad_studies(ctx, request)
