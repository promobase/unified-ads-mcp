...
// ToolAdset_post returns the MCP tool definition for adset_post
func ToolAdset_post() mcp.Tool {
...
// HandleAdset_post handles the adset_post tool with context-based auth
func HandleAdset_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adset_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adset_post: %v", err)), nil
	}
...
// Adset_post performs POST  for AdSet
func Adset_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string

	// Extract ad_set_id for URL construction
	adSetId, ok := args["ad_set_id"].(string)
	if !ok {
		return nil, fmt.Errorf("ad_set_id is required for adset_post")
	}
