...
// ToolAdset_get returns the MCP tool definition for adset_get
func ToolAdset_get() mcp.Tool {
...
// HandleAdset_get handles the adset_get tool with context-based auth
func HandleAdset_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adset_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adset_get: %v", err)), nil
	}
...
// Adset_get performs GET  for AdSet
func Adset_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
