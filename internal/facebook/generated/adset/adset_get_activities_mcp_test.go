...
func TestToolAdset_get_activities_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdset_get_activities()
	if tool.Name != "adset_get_activities" {
		t.Errorf("expected tool name adset_get_activities, got %s", tool.Name)
	}
}

func TestToolAdset_get_activities_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdset_get_activities(ctx, request)
