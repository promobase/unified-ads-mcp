...
// ToolAlbum_get returns the MCP tool definition for album_get
func ToolAlbum_get() mcp.Tool {
...
// HandleAlbum_get handles the album_get tool with context-based auth
func HandleAlbum_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Album_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute album_get: %v", err)), nil
	}
...
// Album_get performs GET  for Album
func Album_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
