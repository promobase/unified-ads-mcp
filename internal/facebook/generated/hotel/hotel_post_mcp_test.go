...
func TestToolHotel_post_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolHotel_post()
	if tool.Name != "hotel_post" {
		t.Errorf("expected tool name hotel_post, got %s", tool.Name)
	}
}

func TestToolHotel_post_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleHotel_post(ctx, request)
