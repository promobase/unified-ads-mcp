...
func TestToolHotel_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolHotel_get()
	if tool.Name != "hotel_get" {
		t.Errorf("expected tool name hotel_get, got %s", tool.Name)
	}
}

func TestToolHotel_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleHotel_get(ctx, request)
