...
func TestToolAdcreative_get_previews_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdcreative_get_previews()
	if tool.Name != "adcreative_get_previews" {
		t.Errorf("expected tool name adcreative_get_previews, got %s", tool.Name)
	}
}

func TestToolAdcreative_get_previews_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdcreative_get_previews(ctx, request)
