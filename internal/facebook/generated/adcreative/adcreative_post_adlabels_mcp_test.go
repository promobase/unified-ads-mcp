...
func TestToolAdcreative_post_adlabels_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAdcreative_post_adlabels()
	if tool.Name != "adcreative_post_adlabels" {
		t.Errorf("expected tool name adcreative_post_adlabels, got %s", tool.Name)
	}
}

func TestToolAdcreative_post_adlabels_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAdcreative_post_adlabels(ctx, request)
