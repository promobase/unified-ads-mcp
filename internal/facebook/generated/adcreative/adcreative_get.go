...
// ToolAdcreative_get returns the MCP tool definition for adcreative_get
func ToolAdcreative_get() mcp.Tool {
	...
	return mcp.NewTool("adcreative_get",
...
// HandleAdcreative_get handles the adcreative_get tool with context-based auth
func HandleAdcreative_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adcreative_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adcreative_get: %v", err)), nil
	}
...
// Adcreative_get performs GET  for AdCreative
func Adcreative_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string

	// Extract ad_creative_id for URL construction
	adCreativeId, ok := args["ad_creative_id"].(string)
	if !ok {
		return nil, fmt.Errorf("ad_creative_id is required for adcreative_get")
	}
