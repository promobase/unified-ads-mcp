...
// ToolCreditcard_get returns the MCP tool definition for creditcard_get
func ToolCreditcard_get() mcp.Tool {
	...
	return mcp.NewTool("creditcard_get",
...
// HandleCreditcard_get handles the creditcard_get tool with context-based auth
func HandleCreditcard_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Creditcard_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute creditcard_get: %v", err)), nil
	}
...
// Creditcard_get performs GET  for CreditCard
func Creditcard_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
