...
func TestToolCanvas_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolCanvas_get()
	if tool.Name != "canvas_get" {
		t.Errorf("expected tool name canvas_get, got %s", tool.Name)
	}
}

func TestToolCanvas_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleCanvas_get(ctx, request)
