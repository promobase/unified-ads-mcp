...
func TestToolPage_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolPage_get()
	if tool.Name != "page_get" {
		t.Errorf("expected tool name page_get, got %s", tool.Name)
	}
}

func TestToolPage_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandlePage_get(ctx, request)
