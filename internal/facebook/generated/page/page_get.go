...
// ToolPage_get returns the MCP tool definition for page_get
func ToolPage_get() mcp.Tool {
...
// HandlePage_get handles the page_get tool with context-based auth
func HandlePage_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Page_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute page_get: %v", err)), nil
	}
...
// Page_get performs GET  for Page
func Page_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
