...
// ToolLeadgenform_get returns the MCP tool definition for leadgenform_get
func ToolLeadgenform_get() mcp.Tool {
	...
	return mcp.NewTool("leadgenform_get",
...
// HandleLeadgenform_get handles the leadgenform_get tool with context-based auth
func HandleLeadgenform_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Leadgenform_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute leadgenform_get: %v", err)), nil
	}
...
// Leadgenform_get performs GET  for LeadgenForm
func Leadgenform_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
