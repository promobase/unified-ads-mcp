...
// ToolStorelocation_get returns the MCP tool definition for storelocation_get
func ToolStorelocation_get() mcp.Tool {
	// Available fields for StoreLocation: full_address, hours, id, phone_number, pickup_options, price_range, store_code, zip_code

	return mcp.NewTool("storelocation_get",
...
// HandleStorelocation_get handles the storelocation_get tool with context-based auth
func HandleStorelocation_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Storelocation_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute storelocation_get: %v", err)), nil
	}
...
// Storelocation_get performs GET  for StoreLocation
func Storelocation_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
