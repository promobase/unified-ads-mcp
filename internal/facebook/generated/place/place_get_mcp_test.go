...
func TestToolPlace_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolPlace_get()
	if tool.Name != "place_get" {
		t.Errorf("expected tool name place_get, got %s", tool.Name)
	}
}

func TestToolPlace_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandlePlace_get(ctx, request)
