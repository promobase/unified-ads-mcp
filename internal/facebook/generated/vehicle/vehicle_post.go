...
// ToolVehicle_post returns the MCP tool definition for vehicle_post
func ToolVehicle_post() mcp.Tool {
	...
	return mcp.NewTool("vehicle_post",
...
// HandleVehicle_post handles the vehicle_post tool with context-based auth
func HandleVehicle_post(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Vehicle_post(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute vehicle_post: %v", err)), nil
	}
...
// Vehicle_post performs POST  for Vehicle
func Vehicle_post(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
