...
// ToolAdstudy_get returns the MCP tool definition for adstudy_get
func ToolAdstudy_get() mcp.Tool {
	...
	return mcp.NewTool("adstudy_get",
...
// HandleAdstudy_get handles the adstudy_get tool with context-based auth
func HandleAdstudy_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Adstudy_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute adstudy_get: %v", err)), nil
	}
...
// Adstudy_get performs GET  for AdStudy
func Adstudy_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
