...
// ToolAsyncsession_get returns the MCP tool definition for asyncsession_get
func ToolAsyncsession_get() mcp.Tool {
	...
	return mcp.NewTool("asyncsession_get",
...
// HandleAsyncsession_get handles the asyncsession_get tool with context-based auth
func HandleAsyncsession_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Asyncsession_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute asyncsession_get: %v", err)), nil
	}
...
// Asyncsession_get performs GET  for AsyncSession
func Asyncsession_get(accessToken string, args map[string]interface{}) (interface{}, error) {
	var baseURL string
