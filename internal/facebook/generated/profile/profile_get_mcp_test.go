...
func TestToolProfile_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolProfile_get()
	if tool.Name != "profile_get" {
		t.Errorf("expected tool name profile_get, got %s", tool.Name)
	}
}

func TestToolProfile_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleProfile_get(ctx, request)
