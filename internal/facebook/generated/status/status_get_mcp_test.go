...
func TestToolStatus_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolStatus_get()
	if tool.Name != "status_get" {
		t.Errorf("expected tool name status_get, got %s", tool.Name)
	}
}

func TestToolStatus_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleStatus_get(ctx, request)
