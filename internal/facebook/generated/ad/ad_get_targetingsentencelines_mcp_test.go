...
func TestToolAd_get_targetingsentencelines_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAd_get_targetingsentencelines()
	if tool.Name != "ad_get_targetingsentencelines" {
		t.Errorf("expected tool name ad_get_targetingsentencelines, got %s", tool.Name)
	}
}

func TestToolAd_get_targetingsentencelines_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := mcp.CallToolParams{
				Name:      "ad_get_targetingsentencelines",
			...
			// Call handler
			result, err := HandleAd_get_targetingsentencelines(ctx, request)
