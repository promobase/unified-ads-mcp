...
func TestToolAd_get_adrules_governed_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolAd_get_adrules_governed()
	if tool.Name != "ad_get_adrules_governed" {
		t.Errorf("expected tool name ad_get_adrules_governed, got %s", tool.Name)
	}
}

func TestToolAd_get_adrules_governed_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleAd_get_adrules_governed(ctx, request)
