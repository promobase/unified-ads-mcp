...
// ToolIguser_get returns the MCP tool definition for iguser_get
func ToolIguser_get() mcp.Tool {
...
// HandleIguser_get handles the iguser_get tool with context-based auth
func HandleIguser_get(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Call the API method
	result, err := Iguser_get(accessToken, args)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("failed to execute iguser_get: %v", err)), nil
	}
...
// Iguser_get performs GET  for IGUser
