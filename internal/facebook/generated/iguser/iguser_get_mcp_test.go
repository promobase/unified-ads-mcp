...
func TestToolIguser_get_Definition(t *testing.T) {
	// Test tool definition
	tool := ToolIguser_get()
	if tool.Name != "iguser_get" {
		t.Errorf("expected tool name iguser_get, got %s", tool.Name)
	}
}

func TestToolIguser_get_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call handler
			result, err := HandleIguser_get(ctx, request)
